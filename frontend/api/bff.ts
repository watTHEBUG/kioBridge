...
const 허용경로 = [
   ...
   * 주문표 경로는 끌 수 있게 둔다.
   *
   * "백엔드가 열려 있으니 여기를 닫아도 같다" 는 한 가지를 빠뜨린 판단이었다.
   * 백엔드를 직접 부르려면 주소를 알아야 하고 CORS 를 넘어야 한다. 이 함수는
   * 그 두 마찰이 없는 창구를 배포 도메인에 하나 더 만든다.
   *
   * 백엔드가 토큰을 줄 때까지, 사고가 나면 배포 환경 변수 하나로 닫을 수 있어야
   * 한다. 로그인은 이 앱에서 선택이라 닫아도 추천부터 최종 확인까지는 그대로
   * 돌아간다 — 저장해 둔 주문표를 불러오는 것만 멈춘다.
   *
   *   KIOBRIDGE_DISABLE_PROFILE_SYNC=1
   *
  ...
  ...(process.env.KIOBRIDGE_DISABLE_PROFILE_SYNC === "1"
    ...
    : [
        ...
        /^api\/v1\/users\/\d+\/profiles\/[^/]+$/,
        // 계정 삭제(#106). 컨트롤러는 아직 없다 — 생기면 바로 통하도록 열어 둔다.
        // DELETE 는 아래 메서드별추가허용에서 연다.
        /^api\/v1\/users\/\d+$/,
      ]),
...
const 메서드별추가허용: { 메서드: string; 경로: RegExp }[] = [
  ...
  // 계정 삭제(#106). 계정 하나만 — 목록 경로로는 DELETE 가 못 간다.
  { 메서드: "DELETE", 경로: /^api\/v1\/users\/\d+$/ },
];
