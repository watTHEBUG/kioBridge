...
export const EN: Record<string, string> = {
   ...
   * 앱 이름.
   *
   * 예전에는 안 옮겼다 — 고유명사라서다. 그런데 영어 화면에 우리말이 하나도
   * 없어야 한다는 쪽으로 정했다. 로고의 aria-label 은 소리로만 듣는 사람에게
   * "키오브릿지" 로 읽히고 있었다.
   *
   * **언어 이름은 같이 넣지 않는다.** 한동안 "한국어": "Korean" 이 여기 있었고,
   * 그래서 영어 화면에서 그 칩이 라틴 문자 Korean 으로 떴다. 그 줄을 찾아야 하는
   * 사람은 한국어를 못 읽는 사람이 아니라 **한국어를 읽는 사람**이다 — 영어 화면에
   * 잘못 들어온 뒤 자기 말로 돌아가려는 사람이라, 자기 글자로 적혀 있어야 찾는다
   * (api/a11y.ts 의 언어목록 주석).
   *
   * "영어 화면에 우리말이 하나도 없어야 한다" 와 부딪히지만 이 한 줄은 예외로 둔다.
   * 나머지가 다 영어여도 되돌아오는 길은 남아야 한다.
   *
   * 표에서 뺀 것만으로는 안 지켜진다 — 사용자가 같은 말을 적을 수 있다. 칩 쪽에
   * data-원문 을 함께 붙였다(app/ui.tsx 의 RadioChip).
   */
  "키오브릿지": "KioBridge",
  // ─── 첫 화면 · 동의 ───────────────────────────────────────────────────────
  ...
  // 동의한 뒤로 바뀌는 줄. 이 줄이 붙어야 소리로 쓰는 분이 체크된 것을 안다.
  "확인하셨어요. 이제 시작하실 수 있어요": "Confirmed. You can start now",
  ...
  //
  // "한국어" 는 일부러 안 넣는다 — 언어를 그 언어로 적는 목록이라 영어 화면에서도
  // 한국어로 보여야 찾을 수 있다(위 '앱 이름' 칸 주석에 자세히). "키오브릿지" 는
  // 반대로 넣기로 했고 위에 있다 — 이 줄이 예전 결정을 그대로 두고 있어서, 표는
  // 바뀌었는데 주석만 남아 같은 파일이 서로 다른 말을 하고 있었다.
  "주요 메뉴": "Main menu",
