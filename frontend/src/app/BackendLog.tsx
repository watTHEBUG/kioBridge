...
const 경로표: 경로설명[] = [
  {
    경로: "/internal/spicy-level/match",
    이름: "맵기 매핑",
    담당: "kjp0411",
    컨트롤러: "SpicyLevelController",
    역할: "말한 맵기 표현을 앵커 표현과의 임베딩 유사도로 골라 준다. 화면 보기에 없는 말(\"불닭맛\")도 잡는다. 확신하지 못하면 고르지 않고 후보와 되물을 문장을 함께 준다.",
    화면: "말로 주문표를 채울 때 맵기 칸. confident 면 그 값이 들어가고, 아니면 '혹시 보통맛 / 순한맛 말씀이신가요?' 로 되묻는다 — 후보가 하나여도 우리가 고르지 않는다.",
  },
  {
    경로: "/api/v1/voice/transcribe",
    이름: "음성 인식",
    담당: "parkseyoung",
    컨트롤러: "VoiceController",
    역할: "녹음한 소리를 멀티파트로 받아 받아 적은 글을 돌려준다(Whisper). 브라우저 음성 인식을 안 쓰는 이유는 Safari 에서 동작하지 않아서다.",
    화면: "'말하기' 로 답하는 모든 칸. 여기서 받은 글이 말 맞추기(voice.ts)를 지나 보기와 대조되고, 맵기는 못 맞추면 위 맵기 매핑으로 간다.",
  },
  {
 ...
 *
 * 어느 저장소에도 담지 않는다. 이용 기록(api/session.ts)에도 얹지 않는다.
 * 여기에는 서버와 오간 본문이 그대로 들어 있어서, 화면이 "저장하는 것" 이라고
 * 밝힌 목록에 없는 것들이 섞여 있다. 확인하려고 만든 화면이 사용자에게 말하지
 * 않은 것을 몰래 남기면 그게 가장 앞뒤가 안 맞는다.
 */
