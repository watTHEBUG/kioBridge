      ...
      <div style={{ padding: `0 ${GAP.screenX}px 32px`, flexShrink: 0 }} className="flex flex-col gap-3">
        ...
        */}
        ...
        {/*
          ...
          여기 있던 안내가 일곱 개까지 늘어 있었다(확인하셔야 · 마이크 허락 ·
          마이크 막힘 · 동의 물음 · 동의 그만함 · 부르는 말 기다림 · 부르기
          그만함). 여러 개가 한꺼번에 뜰 수도 있어서, 화면은 글로 덮이고 정작
          지금 무엇을 하면 되는지는 그 사이에 묻혔다.

          소리로 듣는 분에게는 더 나빴다. 뜬 줄을 다 읽으므로, 답할 차례가
          왔을 때 안내가 아직 안 끝나 있었다.

          하나만 고른다. 위에 있는 것이 더 급한 것이다 — 지금 대답하면 되는
          물음이 맨 위고, 못 하게 막힌 사정이 그다음이고, 그냥 알려 주는 말이
          마지막이다.
        */}
        {(() => {
          const 물음 =
            동의듣기 === "묻는중"
              ? { 글: "이용에 동의하시겠어요? 동의하시면 \"네\" 라고 말씀해 주세요. 대답은 알아듣기 위해 서버로 한 번 전송됩니다.", 지금답할것: true }
              : 부르기상태 === "기다림"
                ? { 글: "부르는 말을 기다리고 있어요. \"키오브릿지\" 라고 말씀하시면 손대지 않고 시작해요.", 지금답할것: true }
                : 동의듣기 === "그만함"
                  ? { 글: "잘 못 알아들어서 그만 들을게요. 위 확인란을 눌러 주세요." }
                  : 부르기상태 === "그만함"
                    ? { 글: "이제 부르는 말을 듣고 있지 않아요. 아래 \"바로 시작하기\" 를 눌러 주세요." }
                    : !동의함 && 소리켜짐 && (마이크 === "물어봐야함" || 마이크 === "모름")
                      ? { 글: "말로 답하시려면 마이크를 한 번 허락해 주세요. 허락하시면 다음부터는 손대지 않고 시작할 수 있어요.", 허락단추: true }
                      : !동의함 && 소리켜짐 && 마이크 === "막힘" && 들을수있나()
                        ? { 글: "마이크가 막혀 있어 말로는 답할 수 없어요. 주소창 옆 자물쇠에서 마이크를 허용하시면 됩니다." }
                        : !동의함
                          ? { 글: "확인하셔야 시작할 수 있어요" }
                          : null;
          if (!물음) return null;
          return (
            <div
              /*
                role="status" 를 유지한다. 스크린리더가 바뀐 줄을 읽고, 우리
                소리 안내도 이 글을 읽는다 — data-소리조용 을 안 붙이는 이유다.
                화면을 못 보는 분에게는 이 한 줄이 "지금 무엇을 하면 되는지" 의
                유일한 신호다.
              */
              role="status"
              style={{
                borderRadius: RADIUS.card,
                // 지금 대답하면 되는 물음만 바탕을 준다. 나머지는 조용한 글이다 —
                // 다 칠하면 강조가 강조가 아니게 된다.
                backgroundColor: 물음.지금답할것 ? SURFACE : "transparent",
                padding: 물음.지금답할것 ? "12px 14px" : "0",
              }}
