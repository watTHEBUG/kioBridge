...
function VoiceSheetScreen({ 언어, onNext, onBack, 주문할수있나 = false }: {
   ...
   * ── 말로 채운 것만 ────────────────────────────────────────────────────────
   *
   * 이 화면은 보기 칩을 눌러 고를 수도 있다. 손으로 마지막 칩을 누른 분까지
   * 저장으로 밀면, 주문표 이름을 적으려던 사람의 손에서 화면을 빼앗는 셈이다.
   * 말로 채운 마지막 한 칸일 때만 넘어간다 — 입력출처.말로채움() 을 가르는
   * 기준과 같다.
   *
   * ── 안 넘어가는 자리 ──────────────────────────────────────────────────────
   *
        ...
        {소리로주고받나() ? (
          <한칸씩말하기
            ...
            on고르기={(축, 고른것, 말로) => {
              ...
              // 이 한 칸을 말로 채웠는가. 위 자동 저장이 이 값을 본다.
              마지막을말로채웠나.current = 말로;
...
function 도움설정말로채우기({ 언어, 설정, onChange, onDone, on마이크막힘 }: {
  ...
  설정: 도움설정;
  onChange: (한칸: Partial<도움설정>) => void;
  /** 끝까지 답하고 끝났으면 참. 건너뛰기로 끝났으면 거짓 — 다음으로() 주석 참고. */
  onDone: (끝까지답했나: boolean) => void;
  /**
  ...
  /*
   * 끝까지 답하고 끝났는지, 건너뛰다 끝났는지를 갈라서 알려 준다.
   *
   * 부르는 쪽(SetupScreen)이 이 값으로 다음 화면까지 넘길지 정한다. 끝까지
   * 답했으면 넘긴다 — 여기까지 말로 온 분에게 마지막에 '계속하기' 단추 하나를
   * 찾아 누르라고 하면, 그 한 번이 남은 문턱이 된다.
   *
   * 건너뛰기로 끝난 것은 다르다. 그건 이 물음들을 그만 듣겠다는 뜻이지 설정을
   * 마쳤다는 뜻이 아니다 — 손으로 고르는 목록에 내려놓고 거기 머문다.
   */
  const 다음으로 = (답했나 = true) => {
    set못들음(null);
      ...
      <div className="flex-1 overflow-y-auto" style={{ minHeight: 0, padding: `28px ${GAP.screenX}px 24px` }}>
        ...
        {음성모드 ? (
          <도움설정말로채우기
            ...
            onChange={onChange}
            /*
             * 끝까지 답했으면 **다음 화면까지 넘긴다.**
             *
             * 여기까지 말로 온 분은 화면을 못 보고 계실 수 있다. 마지막 물음에
             * 답하고 나서 '계속하기' 단추를 찾아 누르라고 하면, 말로 다 해 놓고
             * 마지막 한 번에서 막힌다 — 이 흐름에서 없애려던 바로 그 문턱이다.
             *
             * 건너뛰기로 끝났으면 안 넘긴다. 그건 물음을 그만 듣겠다는 뜻이지
             * 설정을 마쳤다는 뜻이 아니다. 손으로 고르는 목록에 내려놓는다.
             */
            onDone={(끝까지답했나) => { set음성모드(false); if (끝까지답했나) onNext(); }}
            /*
...
function OrderClarification({
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/*
        이유는 후보 목록 **아래에 한 번** 둔다. 후보마다 밑에 붙이지 않는다 —
        서버가 주는 이유는 응답 하나에 딸린 것이지 후보별로 오지 않아서
        (contracts/Candidate.java 에 이유 칸이 없다), 세 후보 밑에 같은 세 줄을
        되풀이해 놓으면 후보마다 다른 근거가 있는 것처럼 읽힌다.
        후보별로 다른 것은 이미 각 줄에 있다 — '조건 일치' 배지와 '고르신 메뉴와
        달라요' 표시가 그것이다.
      */}
      <고른이유 reasons={reasons} scoredAxes={scoredAxes} />
