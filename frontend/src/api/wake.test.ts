...
describe("우리를 부른 말인가", () => {
  ...
  it("한 글자 어긋난 것도 알아듣는다", () => {
    /*
     * 인식기는 한 글자만 틀린 답을 자주 내놓는다. 그때마다 표에 적어 넣는
     * 것은 끝이 없어서, 한 글자 차이까지는 같은 말로 본다.
     */
    for (const 말 of ["키오브릿찌", "키오프릿지", "키오브랫지", "키오브릿지요", "저기 키오브릿찌"]) {
      expect(불렀나(말), 말).toBe(true);
    }
  });

  it("두 글자 넘게 다르면 안 연다", () => {
    // 부르는 말은 앱을 시작시키는 문이다. 옆 사람 대화에 열리면 안 된다.
    for (const 말 of ["키위 브라우니", "카오스 브릿지", "라디오 브런치"]) {
      expect(불렀나(말), 말).toBe(false);
    }
  });
