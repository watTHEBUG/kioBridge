...
const 서버로보내기 = async (
   ...
   *
   * **응답 본문은 남긴다.** 이 기록을 연동 자료로 쓰기로 해서다 — 상태만
   * 있으면 "요청이 나갔다" 는 알아도 "서버가 이 말을 받아 적었다" 는 못 보인다.
   *
   * ── 그 값이 무엇인지 알고 남긴다 ─────────────────────────────────────────
   *
   * 그 응답이 곧 **그 사람이 방금 한 말**이다. 이름이나 전화번호를 말하면
   * 그대로 적힌다. 이 패널은 배포본에서도 보이므로(build:team) 시연 화면과
   * 녹화에 그대로 찍히고, 브라우저 메모리에 60줄까지 남는다.
   *
   * 자료로 쓰겠다는 결정 위에서 그 값을 치른 것이다. 기록은 이 브라우저
   * 메모리에만 있고 서버로 가지도 저장소에 쓰이지도 않는다 — 오디오를 어디에도
   * 저장하지 않는다는 위 약속은 그대로다.
   *
   * **가리고 싶어지면 한 줄이다.** 적기 의 응답 인자를 빼고 가림: true 로
   * 되돌리면 된다. 그때 남는 것은 상태·걸린시간·오디오 형식·크기·언어인데,
