 ...
 * 이 값이 **아니라고** 말했나.
 *
 * 위 두 표(아니라는말·영어로아니라는말)를 밖에서도 쓸 수 있게 연다. 서버가
 * 골라 준 값을 받을 때 필요하다 — 서버는 임베딩 유사도로 고르는데, 그 방식은
 * 부정을 못 읽는다. "안 매운 거" 안에 "매운" 이 들어 있어서 매운맛과 가깝다고
 * 본다(실측: confident=true, HOT). 우리 쪽 표는 그걸 이미 알고 있으므로,
 * 서버 답이 여기 걸리면 안 받는다.
 *
 * 넓히는 것이 아니라 **반대로 읽는 것을 막는 것**이라, 짐작하지 않는다는
 * 규칙과 어긋나지 않는다.
 */
export const 아니라고했나 = (들은말: string, 값: string, 영어인가 = false): boolean => {
  const 글 = 들은말.replace(/\s+/g, " ").trim();
  if (영어인가) {
    const 소문자 = 글.toLowerCase();
    return (영어로아니라는말[값] ?? []).some((말) => 낱말로있나(소문자, 말));
  }
