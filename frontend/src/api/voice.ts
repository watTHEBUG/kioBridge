...
export const 예아니오 = (들은말: string, 영어인가 = false): boolean | null => {
  ...
  const 예 = 영어인가
    ...
    /*
     * 인식기가 실제로 내놓는 어긋남까지 받는다.
     *
     * "네" 는 한 글자라 인식기가 자주 다르게 적는다 — '내' 로 오는 일이 잦고,
     * '넵'·'옙'·'예스' 도 사람이 실제로 쓰는 말이다. 아래 낱말 검사가 앞뒤
     * 공백·마침표를 요구하므로, '내일'·'내가' 같은 말에는 안 걸린다.
     */
    : ["네", "내", "넵", "옙", "예", "예스", "응", "그래", "좋아", "맞아", "해 주", "해줘"];
  /*
   ...
   */
  return 예.some((말) => (영어인가 ? 낱말로있나(글, 말) : new RegExp(`(^|\\s)${말}($|\\s|\\.|,)`).test(글)))
    ? true
    : null;
};
