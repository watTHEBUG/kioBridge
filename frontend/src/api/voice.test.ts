...
import { 말에서고르기, 말로채울수있나, 예아니오, 뒤로가자고했나, 다음가자고했나 } from "./voice";
...
describe("다음으로 가 달라는 말", () => {
  /*
   * 여러 개 고르는 칸(카페의 시럽)에서는 이 말이 없으면 나갈 길이 없다.
   * 값을 넣어도 그 자리에 머무는 칸이라, 손을 안 쓰는 분은 같은 질문만
   * 되풀이해 듣다가 이어 듣기가 꺼진다(#145 리뷰).
   */
  it("다음으로 가자는 말을 알아듣는다", () => {
    for (const 말 of ["다음", "다음이요", "됐어요", "됐습니다", "충분해요", "넘어가 주세요"]) {
      expect(다음가자고했나(말)).toBe(true);
    }
  });

  it("보기 이름과 평범한 답은 명령으로 읽지 않는다", () => {
    for (const 말 of ["바닐라", "헤이즐넛", "카라멜", "시럽 없음", "매운맛", "포장하기", "네", "두 개"]) {
      expect(다음가자고했나(말)).toBe(false);
    }
  });

  it("영어는 낱말로 있을 때만 본다", () => {
    expect(다음가자고했나("next", true)).toBe(true);
    expect(다음가자고했나("that is enough", true)).toBe(true);
    // 'nextdoor' 안의 next 가 걸리면 안 된다.
    expect(다음가자고했나("nextdoor", true)).toBe(false);
    expect(다음가자고했나("vanilla", true)).toBe(false);
  });

  it("뒤로와 겹치지 않는다", () => {
    // 둘 다 참이면 앞의 검사(뒤로)가 이겨서 다음으로 못 간다.
    for (const 말 of ["다음", "됐어요", "넘어가"]) expect(뒤로가자고했나(말)).toBe(false);
    for (const 말 of ["뒤로", "이전", "되돌려"]) expect(다음가자고했나(말)).toBe(false);
  });

  it("빈 말은 아니다", () => {
    expect(다음가자고했나("")).toBe(false);
    expect(다음가자고했나("   ")).toBe(false);
  });
});
