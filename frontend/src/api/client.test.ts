...
import { api, clearSheets, registerSheet, setMockDelays, setScenario, unregisterSheet } from "./client";

// 목의 주문표 보관소는 실제 백엔드의 주문표 저장소 자리다.
// 화면의 '이 기기에서 정보 지우기'는 "지금까지 입력한 내용을 모두 지워요" 라고 약속한다.
// 여기 사본이 남으면 그 문장이 사실이 아니게 되므로, 지우는 경로를 함께 검사한다.

const 주문표 = (id: string, selections: Record<string, string[]>): OrderSheet => ({
  id,
  menuName: "닭강정",
  place: "음식점",
  selections,
...
const 매운 = 주문표("p1", { "이용 방식": ["포장하기"], "맵기": ["매운맛"], "형태": ["순살"] });
const 순한 = 주문표("p2", { "이용 방식": ["포장하기"], "맵기": ["순한맛"], "형태": ["순살"] });
// 순한맛+뼈 조합은 오늘 메뉴에 없다. 실제로 어긋나므로 시나리오 스위치와 무관하게
// changed 가 된다. 목은 없는 불일치를 지어내지 않으므로 이 주문표가 필요하다.
const 안맞음 = 주문표("p3", { "이용 방식": ["포장하기"], "맵기": ["순한맛"], "형태": ["뼈"] });
...
describe("주문표 등록", () => {
  it("등록한 주문표의 조건이 응답에 반영된다", async () => {
    registerSheet(매운);
    expect(await 이유("p1")).toContain("매운맛");
  });

  it("서로 다른 주문표는 서로 다른 답을 낸다", async () => {
    registerSheet(매운);
    registerSheet(순한);
    expect(await 이유("p1")).toContain("매운맛");
...
describe("등록되지 않은 주문표로는 답을 만들지 않는다", () => {
  ...
  it("unregisterSheet 이후에는 그 id 로 답을 만들지 않는다", async () => {
    registerSheet(매운);
    expect(await 이유("p1")).toContain("매운맛");
  ...
  it("unregisterSheet 은 지정한 것만 지운다", async () => {
    registerSheet(매운);
    registerSheet(순한);
    unregisterSheet("p1");
  ...
  it("없는 id 를 지워도 터지지 않는다", () => {
    expect(() => unregisterSheet("없는id")).not.toThrow();
  });

  it("forgetAll 은 계약의 삭제 경로다 — 목 전용 함수를 화면이 부르지 않아도 된다", async () => {
    // 화면이 clearSheets 를 직접 부르면 실제 client 로 바꾸는 순간
    // 그 호출은 목의 Map 만 비우고 서버 데이터는 남는다.
    registerSheet(매운);
    await api.requestMapping(PAIRING, "p1");
    const { planId } = await api.approve({ pairingId: PAIRING, sheetId: "p1", mappingResult: "exact" });
  ...
  it("이미 만든 실행 계획도 함께 지운다", async () => {
    ...
    // 주문표만 지우고 이건 남겨 두면 '모두 지워요' 가 반쯤만 사실이 된다.
    registerSheet(매운);
    await api.requestMapping(PAIRING, "p1");
    const { planId } = await api.approve({ pairingId: PAIRING, sheetId: "p1", mappingResult: "exact" });
...
describe("장바구니는 승인한 내용과 같아야 한다", () => {
  ...
  it("수량을 2개로 저장했으면 금액도 두 배가 된다", async () => {
    registerSheet(주문표("p3", { "이용 방식": ["포장하기"], "맵기": ["매운맛"], "형태": ["순살"], "수량": ["2개"] }));
    const m = await api.requestMapping(PAIRING, "p3");
    const { planId } = await api.approve({ pairingId: PAIRING, sheetId: "p3", mappingResult: "exact" });
    const s = await 담길때까지(planId);
  ...
  it("후보를 고르는 화면에서도 수량이 지켜진다", async () => {
    ...
    registerSheet(주문표("p4", { "이용 방식": ["포장하기"], "맵기": ["매운맛"], "형태": ["순살"], "수량": ["3개"] }));
    const m = await api.requestMapping(PAIRING, "p4");
    const 고른것 = m.candidates![0];
    const { planId } = await api.approve({
      pairingId: PAIRING, sheetId: "p4", mappingResult: "clarification", candidateId: 고른것.candidateId,
    });
    const s = await 담길때까지(planId);
  ...
  it("여러 후보에서 고른 것이 그대로 담긴다", async () => {
    ...
    const { planId } = await api.approve({
      pairingId: PAIRING, sheetId: "p1", mappingResult: "clarification", candidateId: 고른것.candidateId,
    });
    const s = await 담길때까지(planId);
...
describe("승인 검사 — 서버가 자기 답을 기준으로 본다", () => {
  it("매핑을 하기 전에는 승인할 수 없다", async () => {
    ...
    registerSheet(매운);
    await expect(
      api.approve({ pairingId: "안한페어링", sheetId: "p1", mappingResult: "exact" }),
    ).rejects.toThrow();
  });
  ...
  it("clarification 인데 후보를 안 고르면 거절한다", async () => {
    ...
    await api.requestMapping(PAIRING, "p1");
    await expect(
      api.approve({ pairingId: PAIRING, sheetId: "p1", mappingResult: "clarification" }),
    ).rejects.toThrow();
  });
  ...
  it("우리가 주지 않은 후보를 담아 달라고 하면 거절한다", async () => {
    ...
    await api.requestMapping(PAIRING, "p1");
    await expect(
      api.approve({ pairingId: PAIRING, sheetId: "p1", mappingResult: "clarification", candidateId: "c99" }),
    ).rejects.toThrow();
  });
  ...
  it("우리가 준 후보면 승인된다", async () => {
    ...
    const { planId } = await api.approve({
      pairingId: PAIRING, sheetId: "p1", mappingResult: "clarification", candidateId: id,
    });
    expect(planId).toMatch(/^pln_/);
  });

  it("changed 인데 확인 표시가 없으면 거절한다", async () => {
    // 실제로 어긋나는 주문표를 쓴다. 스위치만 changed 로 돌리고 다 맞는 주문표를
    ...
    await expect(
      api.approve({ pairingId: PAIRING, sheetId: "p3", mappingResult: "changed" }),
    ).rejects.toThrow();
  });
  ...
  it("low_confidence 인데 직접 짚지 않으면 거절한다", async () => {
    ...
    await api.requestMapping(PAIRING, "p1");
    await expect(
      api.approve({ pairingId: PAIRING, sheetId: "p1", mappingResult: "low_confidence" }),
    ).rejects.toThrow();
  });
  ...
  it("not_found 는 어떤 경우에도 거절한다", async () => {
    ...
    await api.requestMapping(PAIRING, "p1");
    await expect(
      api.approve({ pairingId: PAIRING, sheetId: "p1", mappingResult: "exact" }),
    ).rejects.toThrow();
  });

  it("매핑한 주문표와 다른 주문표로는 승인할 수 없다", async () => {
    // A 로 찾아 놓고 B 를 담는 걸 막는다.
    registerSheet(매운);
    registerSheet(순한);
    await api.requestMapping(PAIRING, "p1");
    await expect(
      api.approve({ pairingId: PAIRING, sheetId: "p2", mappingResult: "exact" }),
    ).rejects.toThrow();
  });
  ...
  it("같은 매핑으로 두 번 담을 수 없다", async () => {
    ...
    registerSheet(매운);
    await api.requestMapping(PAIRING, "p1");
    await api.approve({ pairingId: PAIRING, sheetId: "p1", mappingResult: "exact" });
    await expect(
      api.approve({ pairingId: PAIRING, sheetId: "p1", mappingResult: "exact" }),
    ).rejects.toThrow();
  });
  ...
  it("동시에 들어온 승인 두 건 중 하나만 담긴다", async () => {
    ...
    registerSheet(매운);
    await api.requestMapping(PAIRING, "p1");
    const 요청 = { pairingId: PAIRING, sheetId: "p1", mappingResult: "exact" as const };
  ...
  it("거절하면 그 세션으로는 다시 승인할 수 없다", async () => {
    ...
    registerSheet(매운);
    await api.requestMapping(PAIRING, "p1");
    await api.reject({ pairingId: PAIRING, sheetId: "p1" });
    await expect(
      api.approve({ pairingId: PAIRING, sheetId: "p1", mappingResult: "exact" }),
    ).rejects.toThrow();
  });

  it("다른 주문표로는 거절할 수 없다", async () => {
    registerSheet(매운);
    registerSheet(순한);
    await api.requestMapping(PAIRING, "p1");
    await api.reject({ pairingId: PAIRING, sheetId: "p2" });
  ...
  it("클라이언트가 mappingResult 를 속여도 서버 판단을 따른다", async () => {
    ...
    await expect(
      api.approve({ pairingId: PAIRING, sheetId: "p3", mappingResult: "exact" }),
    ).rejects.toThrow();
  });
});
