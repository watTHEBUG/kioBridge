...
/**
 * 축이 **맞았을 때** 쓰는 문장. 실서버와 같은 틀로 쓴다.
 *
 * 화면은 축마다 한 줄씩 오는 이 문장들을 한 줄로 합친다(i18n/reason.ts 의
 * 이유묶기). 그런데 그 합치기는 실서버 문장 틀을 알아보고 동작한다 —
 *
 *   선호하신 {축}과 일치하는 메뉴라 우선 추천드립니다.
 *
 * 목이 제 말투로 쓰면 그 틀에 안 걸려서 **목에서만 안 합쳐진다.** 실제로
 * 그랬다 — 실서버는 한 줄인데 목과 시연 화면은 세 줄이었다. 목은 서버를
 * 흉내 내는 자리라, 말투가 다르면 여기서 본 것이 배포본에서 달라진다.
 *
 * 못 맞춘 축("오늘은 그 조합이 없어서…")은 이 틀을 안 쓴다. 그건 맞았다는
 * 말이 아니라서 합쳐지면 안 된다.
 */
const 과와 = (w: string) =>
  // 영문 값(HOT·ICE·Tall)은 한글 받침으로 못 가른다. 읽는 소리를 따른다 —
  // 'HOT'은 '핫'이라 받침이 있고, 'ICE'는 '아이스'라 없다. 로으로() 와 같은 판단이다.
  /[a-zA-Z]$/.test(w) ? (/[aeiouAEIOU]$/.test(w) ? "와" : "과") : 조사(w, "과", "와");

const 맞았다 = (값: string): string =>
  `선호하신 ${값}${과와(값)} 일치하는 메뉴라 우선 추천드립니다.`;
...
function 반영한이유(p: OrderSheet | undefined, 고름: 후보 | undefined): RecommendationReason[] {
  ...
  if (이용방식) out.push({ kind: "used", text: 맞았다(이용방식) });
  if (맵기) {
    out.push(고름?.맵기 === 맵기
      ? { kind: "used", text: 맞았다(맵기) }
      : { kind: "used", text: `맵기를 ${로으로(맵기)} 저장해 두셨는데 오늘은 그 조합이 없어서 가장 가까운 걸로 골랐어요` });
  }
  if (형태) {
    out.push(고름?.형태 === 형태
      ? { kind: "used", text: 맞았다(형태) }
