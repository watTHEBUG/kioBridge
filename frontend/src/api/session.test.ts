...
describe("남길 것이 없으면 아무것도 안 쓴다", () => {
  ...
  it("어느 칸이든 기본과 같으면 남길 것으로 안 센다", () => {
    /*
     * '건드렸다' 는 **기본과 다르다** 는 뜻이지 '켜져 있다' 는 뜻이 아니다.
     *
     * 값이 true 인지로 보던 때가 있었고, 그 방식은 기본값이 falsy 하다는
     * 가정 위에 있었다. language 가 들어오면서 한 번 깨졌고("ko-KR" 이
     * truthy), voiceGuide 를 잠깐 기본 켜짐으로 두었을 때 또 깨졌다 —
     * 아무것도 안 한 사람의 빈 이용이 전부 저장됐다.
     *
     * 지금은 voiceGuide 가 다시 꺼짐이라 겉으로는 차이가 잘 안 난다. 그래도
     * language 는 여전히 "ko-KR"(truthy)이라 옛 방식이었다면 그 칸 하나로
     * 계속 깨졌을 것이고, 기준을 바꿔 둔 덕에 다음에 어떤 칸의 기본값이
     * 바뀌어도 안 깨진다.
     */
    이어쓰기.쓰기({ ...빈값, a11y: { ...기본도움설정 } });
    expect(저장소.length).toBe(0);
  });

  it("소리 안내의 기본값은 꺼짐이다", () => {
    /*
     * 아래 시험들은 기본도움설정 을 그대로 바탕으로 쓴다. 그래서 이 값이
     * 잘못 켜지면 시험 입력도 같이 바뀌어 아무도 못 잡는다 — 기본값 자체를
     * 여기서 못 박는다.
     *
     * 켜 두면 눈으로 보는 대부분의 사람이 열자마자 읽는 소리를 듣는다.
     * 켜는 일은 사람이 한다(api/a11y.ts 의 기본도움설정 주석).
     */
    expect(기본도움설정.voiceGuide).toBe(false);
  });

  it("스위치를 켜면 그 선택은 남긴다", () => {
    // 소리로 듣고 답하기를 켠 사람은 건드린 것이다. 다음에 열었을 때 꺼져
    // 있으면 그분은 첫 화면에서 또 켜야 한다.
    이어쓰기.쓰기({ ...빈값, a11y: { ...기본도움설정, voiceGuide: true } });
    expect(이어쓰기.읽기()!.a11y.voiceGuide).toBe(true);
  });
