...
describe("남길 것이 없으면 아무것도 안 쓴다", () => {
  ...
  it("기본 켜짐인 설정을 안 건드린 것도 남길 것으로 세지 않는다", () => {
    /*
     * '건드렸다' 는 기본과 다르다는 뜻이지 '켜져 있다' 는 뜻이 아니다.
     *
     * voiceGuide 가 기본 켜짐이 되면서 이 자리가 깨졌다 — 값이 true 인지로
     * 보고 있었던 탓에, 아무것도 안 한 사람의 빈 이용이 전부 저장됐다.
     * language 때와 같은 원인이라(기본값이 falsy 하다는 가정) 예외를 더하는
     * 대신 기준을 바꿨다.
     */
    expect(기본도움설정.voiceGuide).toBe(true);
    이어쓰기.쓰기({ ...빈값, a11y: { ...기본도움설정 } });
    expect(저장소.length).toBe(0);
  });

  it("기본 켜짐인 것을 끄면 그 선택은 남긴다", () => {
    // 끈 사람은 건드린 것이다. 다음에 열었을 때 다시 켜져 있으면 안 된다.
    이어쓰기.쓰기({ ...빈값, a11y: { ...기본도움설정, voiceGuide: false } });
    expect(이어쓰기.읽기()!.a11y.voiceGuide).toBe(false);
  });
