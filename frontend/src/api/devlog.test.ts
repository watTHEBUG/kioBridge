...
describe("직접 부르는 경로도 기록에 남는다", () => {
  /*
   * 맵기 매칭과 음성 인식은 createTeamBackend 를 안 거치고 직접 부른다. 그래서
   * 개발 패널에 아무 줄도 안 떴고, 서버에 붙은 것인지 아닌지 눈으로 알 수
   * 없었다 — 그 패널을 만든 이유가 바로 그걸 보는 것인데.
   *
   * 대신 **본문은 안 남긴다.** 두 경로 다 사용자가 방금 한 말이 실린다.
   * '가림' 으로 표시해서, 보는 사람이 '비어 있는 것' 과 '일부러 안 남긴 것' 을
   * 헷갈리지 않게 한다.
   */
  it("본문 없이 남긴 것은 가림으로 표시된다", () => {
    연동기록.비우기();
    연동기록.남기기({
      방법: "POST", 경로: "/api/bff/internal/spicy-level/match",
      상태: 200, 걸린시간: 12, 시각: Date.now(), 가림: true,
    });
    const [한줄] = 연동기록.읽기();
  ...
  it("실패도 남는다 — 안 뜨면 안 부른 것과 구분이 안 된다", () => {
    연동기록.비우기();
    연동기록.남기기({
      방법: "POST", 경로: "/api/bff/api/v1/voice/transcribe",
      상태: "실패", 걸린시간: 8000, 시각: Date.now(), 가림: true,
    });
    expect(연동기록.읽기()[0].상태).toBe("실패");
  });
});
