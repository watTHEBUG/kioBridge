 ...
 *
 * 둘뿐인 이유 — 이제 앱 화면이 실제로 그 언어로 바뀐다(src/i18n). 화면에 나가는
 * 문장을 전부 옮겨야 하는 일이라, 옮길 준비가 된 언어만 목록에 올린다.
 * 고를 수는 있는데 화면은 한국어인 언어를 두면 그게 가장 나쁘다.
 */
export const 언어목록 = [
...
/**
 * 소리로 듣고 답하기를 **처음부터 켜 둔다.**
 *
 * 여태 꺼짐이었다. 그런데 이 스위치는 화면을 못 보는 분에게 가장 필요한
 * 것인데, 켜려면 화면에서 그 스위치를 찾아 눌러야 했다 — **읽어 줘야 읽을
 * 수 있는 사람은 켜러 갈 수가 없다.** 첫 화면으로 옮겨 놓은 것도 그래서였고,
 * 이번에 그 마지막 한 걸음까지 없앤다.
 *
 * 맞바꾸는 것이 있다. 눈으로 보는 분에게는 열자마자 화면을 읽는 소리가 난다.
 * 그 대신 끄는 길은 늘 첫 화면 맨 위에 있고, 한 번 끄면 그 설정이 남는다.
 * 소리를 못 내는 기기에서는 스위치 자체가 안 보이므로 달라지는 것이 없다.
 *
 * 마이크가 저절로 열리지는 않는다. 브라우저가 이미 허용해 둔 자리에서만
 * 부르는 말을 기다리고(api/wake.ts), 그 전에는 아무것도 듣지 않는다.
 */
export const 기본도움설정: 도움설정 = { ...기본접근성, voiceGuide: true, language: "ko-KR" };
