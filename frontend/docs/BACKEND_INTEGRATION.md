...
화면에 '계정 삭제' 가 생겼습니다. 지금은 이 기기의 정보와 서버 주문표(`DELETE /users/{id}/profiles/{pid}`)까지는 지우는데, **계정 자체를 지우는 경로가 없어서** 아이디가 서버에 남습니다. 화면은 그 사실을 그대로 말하고 있습니다("계정을 지우는 길이 아직 서버에 없어서 아이디는 남아 있어요").
...
프론트는 이미 이 경로를 부르고, 실패의 종류를 갈라 안내합니다 — 404·405 는 "지우는 길이 아직 없다", 그 밖의 실패는 "이번에 못 지웠으니 다시 로그인해 시도해 달라". **안내문이 사라지는 것은 경로가 존재할 때가 아니라 삭제가 2xx 로 성공했을 때입니다.**
