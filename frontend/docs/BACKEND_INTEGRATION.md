...
- 이 앱이 저장하는 것은 **주문 조건뿐**입니다 — 장소·맵기·형태·컵·수량·알레르기·메모. **실명·전화번호·주소를 받는 칸이 하나도 없습니다**(심사 요건이기도 합니다). 호칭은 이 기기 밖으로 나가지 않습니다.
- 다만 **메모는 자유 입력이라 그 원칙에 뚫린 구멍입니다.** 그래서 저장 전에 모양으로 거릅니다(`개인정보같은메모`, `src/api/account.ts`). 무엇을 막고 못 막는지 그대로 적습니다.

  | | |
  | --- | --- |
  | 막습니다 | 전화번호, 주민등록번호, 주소로 보이는 것(행정구역 + 번지·호) |
  | **못 막습니다** | **사람 이름, 환자번호** — 정해진 모양이 없어 정규식으로 가려낼 수 없습니다 |

  걸러도 지우지는 않습니다. 앱이 말없이 고치면 화면에 보이는 것과 저장되는 것이 달라집니다. 무엇을 지워야 하는지 말해 주고 사용자가 고칩니다. **"개인정보를 저장하지 않는다" 를 이 검사로 보장한다고 적지 않는 이유가 이것입니다** — 보장하는 것은 받는 칸이 없다는 사실이고, 메모는 거기서 예외입니다.
...
화면의 '이 기기에서 정보 지우기' 는 두 가지를 합니다. 나누는 이유는 지울 수 있는 것과 없는 것이 다르기 때문입니다.

| 누가 | 무엇을 |
| --- | --- |
| `api.forgetAll()` | 연동 계층이 들고 있는 것 — 세션·만료·환경, 정규화된 주문표와 세션 맥락, 화면이 등록해 둔 주문표 사본 |
| `App.tsx` 의 확인창 핸들러 | 화면 상태 — 주문표 목록, 호칭, 로그인 상태, 연결 정보 |

**둘 다 이 기기 안에서만 일어납니다.** 서버에 지우는 경로가 없어서요.

**서버에도 지우는 경로가 있어야 합니다.** 지금은 `POST /api/v1/users/{id}/profiles` 로 올린 주문표를 지울 방법이 없습니다. 화면에서 지워도 서버에는 남고, 다시 로그인하면 그대로 돌아옵니다. 승인·거절 기록도 남습니다. 그래서 개인정보 안내 화면에 이렇게 적어 두었습니다 — 지운 척하지 않으려고요.
...
### `summary.status` 를 문장 말고 코드로 주실 수 있을까요

`#48` 의 `EvidenceSummary.status` 는 지금 완성된 한국어 문장으로 옵니다.

```text
"정상적으로 장바구니에 추가되었습니다."  "실행할 수 없습니다."
"안전하게 중단되었습니다."             "처리 중 문제가 발생했습니다."
```

내용은 정확한데 `~되었습니다` 체라, 처음부터 끝까지 `~해요` 로 말하는 이 앱의 마지막 화면에서만 문체가 바뀝니다. 그래서 프론트가 네 문장을 앱 말투로 옮겨 씁니다(`src/api/backend.ts` 의 `앱말투`). **문장이 곧 키라서, 서버에서 문구를 한 글자만 고쳐도 이 표가 조용히 안 맞게 됩니다.**

`status` 를 `CART_READY` · `NOT_EXECUTABLE` · `SAFETY_STOPPED` · `FAILED` 같은 코드로 주시면(문장은 그대로 두셔도 됩니다) 그 걱정이 없어집니다. `stopType` 만으로는 '실행할 수 없습니다' 를 구분할 수 없어서 이 값이 필요합니다.

### 추천 이유마다 어느 축 얘기인지 알려 주실 수 있을까요

`recommendationReasons` 와 `unmetConditions` 는 지금 `string[]` 입니다. 문장만 오고, 그 문장이 어느 조건(이용 방식·맵기·형태·컵·수량) 얘기인지는 안 옵니다.

프론트는 그 문장 옆에 사용자가 고른 값을 같이 보여 주고 싶었습니다 — "선호하신 맵기와 맞는 메뉴라 **(고르신 값: 매운맛)**" 처럼요. 어느 축인지 모르니 **문장에 축 이름이 들어 있는지로 짐작**했는데, 부분 문자열이라 선택 근거가 아닌 문장에도 붙었습니다. 서버가 재고 안내에서 "컵" 을 한 번 쓰기만 해도 사용자가 고른 컵이 추천 근거인 것처럼 떴습니다. **사용자에게 틀린 근거를 보여 주는 것이라 짐작을 걷어냈습니다**(`src/api/backend.ts`). 지금은 서버 문장을 그대로만 보여 줍니다.

정확 일치로 바꾸는 길도 생각했지만, 비교할 문장 표가 없습니다. 킷 계약의 정의가 `"<사용자가 읽을 이유>"` 라 문구가 자유롭고, 킷 예제의 표준 문장(`"사용자 조건에 맞는 항목입니다."`)에는 축 단어가 아예 없습니다.

이유 한 줄마다 **축 식별자**를 같이 주시면 그대로 붙일 수 있습니다. 킷 `environments/chicken-store/compatibility-rules.json` 이 이미 쓰는 키를 그대로 쓰시면 됩니다.

```text
SERVICE_TYPE   SPICY_LEVEL   BONE_TYPE   CUP   quantity
```

예를 들면 이런 모양입니다(문장은 지금 그대로 두셔도 됩니다).

```json
"recommendationReasons": [
  { "text": "선호하신 맵기와 맞는 메뉴예요.", "optionKey": "SPICY_LEVEL" }
]
```

`ruleId`(`CHICKEN_SPICY_LEVEL_PREFERENCE` 같은 것)를 주셔도 됩니다 — 프론트가 규칙 식별자를 화면에 띄우지는 않고, 축을 찾는 데만 씁니다. 어느 쪽이든 문자열 배열을 그대로 두고 칸을 하나 더 얹는 형태면 충분합니다.

### ✅ 계정을 지우는 경로 (팀 #118, 해결됨)

```text
DELETE /api/v1/users/{userId}
→ 204 (Bearer 인증 + 소유자 검증, 딸린 주문표도 함께 삭제)
```

요청한 그대로 나왔습니다. 특히 **이미 없는 계정도 204** 로 답해 주신 것이 중요했습니다 — 404 였다면 프론트가 "지우는 길이 아직 없다" 로 잘못 읽었을 자리입니다. 주문표를 DB 캐스케이드에만 맡기지 않고 먼저 지워 주신 것도 그대로 확인했습니다.

프론트는 실패의 종류를 갈라 안내합니다 — 404·405 는 "지우는 길이 아직 없다", 그 밖의 실패는 "이번에 못 지웠으니 다시 로그인해 시도해 달라".

### 도움 설정·알레르기를 계정에 저장할 자리가 필요합니다

주문표는 로그인하면 서버에서 돌아오는데, 도움 설정(큰 글씨·고대비…)과 알레르기는 이 기기에만 남습니다. 다른 기기에서 로그인한 사람이 알레르기를 다시 고르다 빠뜨리면 그 주문은 안 걸러집니다 — 주문표보다 오히려 이쪽이 계정에 남아야 할 값입니다. 킷 안내문의 '로그인 개인화' 확장 항목이기도 합니다(접근성 설정과 사용자 선호 저장).

주문표 API 와 같은 꼴을 제안합니다 (Bearer 인증 + 소유자 검증 그대로):

```text
GET /api/v1/users/{userId}/preferences
→ 200 { "accessibility": { "largeText": false, …아홉 칸 }, "allergies": ["PEANUT", …] }
→ 404 저장한 적 없음
...
- `accessibility` 는 화면의 도움 설정 아홉 칸 전부입니다(계약 일곱 칸 + voiceGuide + language). 키오스크 계약으로 내보낼 때 일곱 칸만 거르는 것은 지금처럼 프론트가 합니다(canonical.ts 의 일곱칸만).
- `allergies` 는 아는 여섯 enum 만 받아 주세요(PEANUT SOY MILK EGG WHEAT SHRIMP). 모르는 값은 400 으로 막아 주시면 됩니다.
- **프론트는 이미 이 경로를 부르고 있습니다.** 로그인하면 GET 하고, 설정을 바꾸면 PUT 합니다. 경로가 없는 지금은 실패를 조용히 삼키고 기기 값으로 동작하므로(claimCode 와 같은 방식), 서버가 받기 시작하는 날 그냥 켜집니다.
- '이 기기에서 정보 지우기' 는 여기에 따로 쓰지 않습니다. 그 흐름은 위의 계정 삭제를 부르고, 계정이 지워지면 딸린 프로필도 함께 사라지기 때문입니다. **계정을 지우실 때 preferences 도 같이 지워 주세요.**
